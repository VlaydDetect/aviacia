...
`telemetry.csv` schema v3 содержит весь полёт: raw/SI-входы, фактически сформированные выходы,
`dt`, `tick_id`, участок, режим/маску, revision конфигурации и локальные времена RX, начала/конца
расчёта и TX. `approach.csv` и `ground.csv` —
...
отправки без повторной сериализации. CSV/JSONL и flush выполняет один writer thread через
ограниченную очередь; в control thread остаются immutable sample/event и публикация dashboard.
В памяти остаётся только ограниченное окно дашборда. Ошибка записи или переполнение очереди видны в manifest,
...
feedback, command-to-feedback метрики ухода, p50/p95/p99 runtime, плавность стыка и
ожидаемую/фактическую диагностику сцепления, ветра и риска
