...
## Почему воздушные PID не входят в legacy-пространство NPGS

`config/regulators.py` фиксирует пять регуляторов старого rollout NPGS/PPO. Активный SFT не
расширяет этот 17-мерный action: у воздуха отдельный 9-мерный checkpoint, у земли — отдельный
15-мерный. Оба выдают только `kp/ki/kd`; статический accepted preset остаётся guard/fallback.
"""
...
class ApproachConfig:
    ...
    loc_full_scale_deg: float = 2.5
    """Угловой эквивалент полного отклонения курсового маяка (град): `курс_град = точки · это`.
    Нужен для проверки углового допуска ТЗ по курсу (≤ 0.7°). ТРЕБУЕТ ПОДТВЕРЖДЕНИЯ на стенде —
    зависит от геометрии ILS ВПП; значение по умолчанию — типовой полукурсовой сектор."""
    gs_full_scale_deg: float = 0.7
    """Угловой эквивалент полного отклонения глиссадного маяка (град): `глиссада_град = точки · это`.
    Нужен для проверки углового допуска ТЗ по глиссаде (≤ 0.5/0.7/1°). ТРЕБУЕТ ПОДТВЕРЖДЕНИЯ на
    стенде — типовое полное отклонение глиссады ≈ 0.7°."""
    ...
    go_around_target_vs_fpm: float = 1000.0
    """Целевая вертикальная скорость набора при уходе (фут/мин, вверх). Уставка тангажа ведётся к
    ней тем же `pitch_pid`, что и на заходе, — отдельного закона набора не заводим."""
    go_around_throttle_norm: float = 1.0
    """Взлётный режим: нормированное положение РУД → 1.0 (полный вперёд) максимальным темпом."""
    go_around_roll_target_deg: float = 0.0
    """Уставка крена при уходе: крылья в горизонт."""
    go_around_min_climb_fpm: float = 300.0
    """Порог «набор устойчив»: фактическая вертикальная скорость выше него (вверх)."""
    go_around_min_gain_ft: float = 50.0
    """Порог «набор устойчив» по приросту радиовысоты над точкой входа в манёвр."""
    go_around_max_seconds: float = 12.0
    """Страховочный таймаут манёвра: даже без подтверждённого набора управление отдаётся пилоту,
    а не удерживается бесконечно."""
