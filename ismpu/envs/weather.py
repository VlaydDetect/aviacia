...
class RunwayCondition(Enum):
    ...
    Фактические пакеты стенда используют коды 1, 2, 3, 4, 5, 7 и 14. Их числовой
    порядок **не** отражает скользкость: например, ICE=4 хуже FLOODED=3, а WET
    RUBBER=14 близок к WET=2. Поэтому код переводится в эту шкалу
...
"""Код `ICSInputs.RunwayCondition` из фактических пакетов → наша шкала скользкости.
