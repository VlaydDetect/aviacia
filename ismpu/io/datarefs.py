...
# --- Индикаторы состояния симулятора (готовность после reload планера) ---
# total_flight_time_sec СБРАСЫВАется к 0 при сбросе полёта (reload_aircraft) и снова
# растёт, когда физика пошла → надёжный признак «перезагрузка завершена, сим считает».
TOTAL_FLIGHT_TIME = "sim/time/total_flight_time_sec"  # с; 0 после сброса полёта, растёт при работе
SIM_SPEED_ACTUAL = "sim/time/sim_speed_actual"        # ratio; 0 на паузе/загрузке, >0 когда сим шагает
