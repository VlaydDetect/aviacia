...
class XPlaneConnectX():
    ...
    def subscribeDREFs(self, subscribed_drefs: list[Tuple[str, int]], history: float = 0.0, timeout: float = 5.0,
        ...
        self.current_dref_values = {sdf[0]: {'value': None, 'timestamp': None, "history": []} for sdf in
            ...
            if header[0:4] == b'RREF':
                ...
                for p_idx in range(no_packets):
                    ...
                    if idx in self.reverse_index.keys():  # if not in self.reverse_idx, the received packet is for the getDREF method or a stale subscription
                        ...
                        history = [x for x in history if
                                   x["timestamp"] >= history[0]["timestamp"] - datetime.timedelta(seconds=self.history)]
    ...
    def startRECORDING(self) -> None:
        ...
        if self.recording_in_progress:
            print(
                "Warning: Recording was interrupted by the start of a new recording. Data from the previous recording is lost.")
    ...
    def _synchronize_measurements(self, data_dict, target_frequency_hz=10):
        ...
        start_time = max([min(all_times[k]) for k in
                          all_times.keys()])  # choose the latest start time to avoid extrapolation on any df
        end_time = min([max(all_times[k]) for k in
                        all_times.keys()])  # choose the ealiest stop time to avoid extrapolation on any df
    ...
    def getDREF(self, dref: str) -> float:
        ...
        idx = len(list(
            self.reverse_index.keys())) + 10  # set the index to the highest index of the permanently observed DataRefs + 10
    ...
    def sendPOSI(self, lat: float, lon: float, elev: float, phi: float, theta: float, psi_true: float,
                 ac: int = 0) -> None:
        ...
        msg = struct.pack('<4sxidddfff', b'VEHS',
                          ...
                          phi)
        self.sock.sendto(msg, (self.ip, self.port))
        self.sock.sendto(msg, (self.ip,
                               self.port))  # send twice since the elevation is erroneously calculated based on initial location

    def getPOSI(self) -> Tuple[
        ...
        temp_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        msg = struct.pack('4sx10s', b'RPOS',
                          b'100')  # request at 100Hz, this should be sufficiently low enough latency for most use cases
        temp_socket.sendto(msg, (self.ip, self.port))
        received = False
        while not received:
            data, addr = temp_socket.recvfrom(16348)
